...
package com.mongodb.cosmosspark
...
import com.mongodb.cosmosspark.config.{MongoCollectionConfig, ReadConfig, WriteConfig}
import com.mongodb.cosmosspark.connection.{DefaultMongoClientFactory, MongoClientCache}
...
object MongoConnector {
  ...
  /**
   * Creates a MongoConnector using the [[com.mongodb.cosmosspark.config.ReadConfig.mongoURIProperty]] from the `sparkConf`.
   *
  ...
  /**
   * Creates a MongoConnector using the [[com.mongodb.cosmosspark.config.ReadConfig]] from the `sparkConf`.
   *
  ...
  private[cosmosspark] val mongoClientKeepAlive = Duration(System.getProperty(
...
case class MongoConnector(mongoClientFactory: MongoClientFactory)
   ...
   *
   * @param config the [[com.mongodb.cosmosspark.config.MongoCollectionConfig]] determining which database to connect to
   ...
   *
   * @param config the [[com.mongodb.cosmosspark.config.MongoCollectionConfig]] determining which database and collection to connect to
   * @param code   the code block that is executed
   * @tparam T the result of the code function
   * @return the result
   */
  ...
  private[cosmosspark] def hasSampleAggregateOperator(readConfig: ReadConfig): Boolean = {
  ...
  private[cosmosspark] def acquireClient(): MongoClient = MongoConnector.mongoClientCache.acquire(mongoClientFactory)
  private[cosmosspark] def releaseClient(client: MongoClient): Unit = MongoConnector.mongoClientCache.release(client)
  private[cosmosspark] def codecRegistry: CodecRegistry = withMongoClientDo({ client => client.getMongoClientOptions.getCodecRegistry })
