...
import com.mongodb.cosmosspark.DefaultHelper.DefaultsTo
import com.mongodb.cosmosspark.config.{ReadConfig, WriteConfig}
import com.mongodb.cosmosspark.rdd.MongoRDD
import com.mongodb.cosmosspark.rdd.api.java.JavaMongoRDD
import com.mongodb.cosmosspark.sql.MapFunctions.rowToDocumentMapper
import com.mongodb.cosmosspark.sql.{MongoInferSchema, MongoRelation, helpers}
...
object MongoSpark {
            ...
            while (!done) {
              try {
                ...
                case ex: MongoBulkWriteException =>
                  if ((tries < 10) && ex.getMessage().contains("16500")) {
                    val pattern = "(?s).*RetryAfterMs=([0-9]+).*".r
                    var delay = 100
                    ex.getMessage() match {
                      case pattern(delayStr) =>
  ...
  class Builder {
    ...
    /**
     * Sets the [[com.mongodb.cosmosspark.MongoConnector]] to use
     *
    ...
    /**
     * Sets the [[com.mongodb.cosmosspark.config.ReadConfig]] to use
     *
   ...
   *
   * @param javaRDD        the RDD data to save to MongoDB
   * @param writeConfig the [[com.mongodb.cosmosspark.config.WriteConfig]]
   * @return the javaRDD
   */
