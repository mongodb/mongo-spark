...
import com.mongodb.cosmosspark.annotation.DeveloperApi
 ...
 * :: DeveloperApi ::
 *
 * Helpers to create [[com.mongodb.cosmosspark.rdd.MongoRDD]] in the current `SparkContext`.
 *
...
case class SparkContextFunctions(@transient sc: SparkContext) extends Serializable {
   ...
   * Creates a MongoRDD
   *
   * Defaults to using the `SparkConf` for configuration, alternatively supply a [[com.mongodb.cosmosspark.config.ReadConfig]].
   *
