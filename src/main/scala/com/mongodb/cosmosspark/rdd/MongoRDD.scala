...
package com.mongodb.cosmosspark.rdd
...
import com.mongodb.cosmosspark.exceptions.MongoSparkCursorNotFoundException
import com.mongodb.cosmosspark.rdd.api.java.JavaMongoRDD
import com.mongodb.cosmosspark.rdd.partitioner.{MongoPartition, MongoSinglePartitioner}
import com.mongodb.cosmosspark.{MongoConnector, MongoSpark, NotNothing, classTagToClassOf}
 ...
 * MongoRDD Class
 *
 * @param connector the [[com.mongodb.cosmosspark.MongoConnector]]
 * @param readConfig the [[com.mongodb.cosmosspark.config.ReadConfig]]
 * @tparam D the type of the collection documents
 */
class MongoRDD[D: ClassTag](
    @transient val sparkSession:         SparkSession,
    private[cosmosspark] val connector:  Broadcast[MongoConnector],
    private[cosmosspark] val readConfig: ReadConfig
  ...
  private[cosmosspark] lazy val hasSampleAggregateOperator: Boolean = connector.value.hasSampleAggregateOperator(readConfig)

  private[cosmosspark] def appendPipeline[B <: Bson](extraPipeline: Seq[B]): MongoRDD[D] = withPipeline(readConfig.pipeline ++ extraPipeline)
}
