...
package com.mongodb.dtxspark.rdd
...
import com.mongodb.dtxspark.exceptions.MongoSparkCursorNotFoundException
import com.mongodb.dtxspark.rdd.api.java.JavaMongoRDD
import com.mongodb.dtxspark.rdd.partitioner.{MongoPartition, MongoSinglePartitioner}
import com.mongodb.dtxspark.{MongoConnector, MongoSpark, NotNothing, classTagToClassOf}
 ...
 * MongoRDD Class
 *
 * @param connector the [[com.mongodb.dtxspark.MongoConnector]]
 * @param readConfig the [[com.mongodb.dtxspark.config.ReadConfig]]
 * @tparam D the type of the collection documents
 */
class MongoRDD[D: ClassTag](
    @transient val sparkSession:      SparkSession,
    private[dtxspark] val connector:  Broadcast[MongoConnector],
    private[dtxspark] val readConfig: ReadConfig
  ...
  private[dtxspark] lazy val hasSampleAggregateOperator: Boolean = connector.value.hasSampleAggregateOperator(readConfig)

  private[dtxspark] def appendPipeline[B <: Bson](extraPipeline: Seq[B]): MongoRDD[D] = withPipeline(readConfig.pipeline ++ extraPipeline)
}
