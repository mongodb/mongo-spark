...
import com.mongodb.dtxspark.annotation.DeveloperApi
 ...
 * :: DeveloperApi ::
 *
 * Helpers to create [[com.mongodb.dtxspark.rdd.MongoRDD]] in the current `SparkContext`.
 *
...
case class SparkContextFunctions(@transient sc: SparkContext) extends Serializable {
   ...
   * Creates a MongoRDD
   *
   * Defaults to using the `SparkConf` for configuration, alternatively supply a [[com.mongodb.dtxspark.config.ReadConfig]].
   *
