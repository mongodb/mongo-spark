...
import com.mongodb.dtxspark.config.{MongoCollectionConfig, ReadConfig, WriteConfig}
import com.mongodb.dtxspark.connection.{DefaultMongoClientFactory, MongoClientCache}
...
object MongoConnector {
  ...
  /**
   * Creates a MongoConnector using the [[com.mongodb.dtxspark.config.ReadConfig.mongoURIProperty]] from the `sparkConf`.
   *
  ...
  /**
   * Creates a MongoConnector using the [[com.mongodb.dtxspark.config.ReadConfig]] from the `sparkConf`.
   *
  ...
  private[dtxspark] val mongoClientKeepAlive = Duration(System.getProperty(
...
case class MongoConnector(mongoClientFactory: MongoClientFactory)
   ...
   *
   * @param config the [[com.mongodb.dtxspark.config.MongoCollectionConfig]] determining which database to connect to
   ...
   *
   * @param config the [[com.mongodb.dtxspark.config.MongoCollectionConfig]] determining which database and collection to connect to
   * @param code   the code block that is executed
   * @tparam T the result of the code function
   * @return the result
   */
  ...
  private[dtxspark] def hasSampleAggregateOperator(readConfig: ReadConfig): Boolean = {
  ...
  private[dtxspark] def acquireClient(): MongoClient = MongoConnector.mongoClientCache.acquire(mongoClientFactory)
  private[dtxspark] def releaseClient(client: MongoClient): Unit = MongoConnector.mongoClientCache.release(client)
  private[dtxspark] def codecRegistry: CodecRegistry = withMongoClientDo({ client => client.getMongoClientOptions.getCodecRegistry })
