...
import com.mongodb.cosmosspark.rdd.partitioner.{DefaultMongoPartitioner, MongoShardedPartitioner, MongoSplitVectorPartitioner}
...
class ReadConfigSpec extends FlatSpec with Matchers {
  ...
  it should "set new options when using withOptions" in {
    ...
    val options = Map(
      ...
      "hint" -> """{"a": 1}""",
      "localthreshold" -> "99",
      "partitioner" -> "com.mongodb.cosmosspark.rdd.partitioner.MongoSplitVectorPartitioner$",
